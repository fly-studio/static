...
//     }
	...
	function makeWindowProjection(win_width, win_height, fov_x_radians) {
		...
		var tan_half = Math.tan(fov_x_radians / 2); if (tan_half - 0.99999 < 0.00001) tan_half = 1;
...
var triangles = function(rect, axis, options, drawTriangle){
	var MIN_Z = 0.05;
	var _options = {
		nonadaptive_depth: options && options.nonadaptive_depth || 0,
		subdivide_factor: options && options.subdivide_factor || 10,
		horizontal_fov_radians: options && options.horizontal_fov_radians || Math.PI / 2, //面向脸
		target_distance: options && options.target_distance || 1
	};

	var _axis = axis || {x: 0, y: 0, z: 0};
	axis = jsgl.vectorScale(_axis, 1);

	var vertices = [], indices = [], uvtData = [];

	var c3d = new jsgl.Context(null);
	var temp_mat0 = jsgl.makeIdentityAffine();
	var temp_mat1 = jsgl.makeIdentityAffine();

	var proj_mat = jsgl.makeWindowProjection(rect.contrainerWidth , rect.contrainerHeight, _options.horizontal_fov_radians);//舞台矩阵
	var object_mat = jsgl.makeOrientationAffine({x:0, y:0, z:0}, {x:1, y:0, z:0}, {x:0, y:1, z:0}); //物件矩阵
	var camera_mat = jsgl.makeOrientationAffine({x:0, y:0, z: 0.0 + _options.target_distance}, {x:0, y:0, z:-1}, {x:0, y:1, z:0}); //相机矩阵

	object_mat = rotateObject(axis, object_mat);

	var view_mat = jsgl.makeViewFromOrientation(camera_mat);console.log(proj_mat);
	jsgl.multiplyAffineTo(proj_mat, view_mat, temp_mat0); //合并0
	jsgl.multiplyAffineTo(temp_mat0, object_mat, temp_mat1); //合并到1
	c3d.setTransform(temp_mat1); //Canvas设置

	var verts = [
		{x:-1, y:-1, z: 0, u:0, v:0},
		{x: 1, y:-1, z: 0, u:rect.width, v:0},
		{x: 1, y: 1, z: 0, u:rect.width, v:rect.height},
		{x:-1, y: 1, z: 0, u:0, v:rect.height}
	];
	var tverts = [];
	...
	drawPerspectiveTri(/*c3d, */tverts[0], tverts[1], tverts[2], _options.nonadaptive_depth);
	drawPerspectiveTri(/*c3d, */tverts[0], tverts[2], tverts[3], _options.nonadaptive_depth);
	vertices.forEach(function(v, k){
		indices.push(k);
	});
	return {vertices:vertices, uvtData: uvtData, indices: indices};
	...
	function drawPerspectiveTriUnclippedSub(/*c3d, */v0, tv0, v1, tv1, v2, tv2, depth_count) {
		...
		var subdiv = ((edgelen01 * zdepth01 > _options.subdivide_factor) ? 1 : 0) +
								 ((edgelen12 * zdepth12 > _options.subdivide_factor) ? 2 : 0) +
								 ((edgelen20 * zdepth20 > _options.subdivide_factor) ? 4 : 0);
	...
	/*      jsgl.drawTriangle(c3d.canvas_ctx_, images[0],
													...
													v2.u, v2.v);*/
		 // }
			var x0 = tv0.x, y0 = tv0.y, x1 = tv1.x, y1 = tv1.y, x2 = tv2.x, y2 = tv2.y,
			sx0 = v0.u, sy0 = v0.v, sx1 = v1.u, sy1 = v1.v, sx2 = v2.u, sy2 = v2.v;

		 	vertices.push(x0, y0, x1, y1, x2, y2);
		 	uvtData.push(sx0, sy0, sx1, sy1, sx2, sy2);
			...
			var dy = (sx0 * (sy2 * y1 - sy1 * y2) + sy0 * (sx1 * y2 - sx2 * y1) + (sx2 * sy1 - sx1 * sy2) * y0) / denom;
		 	...
		 	if (drawTriangle) drawTriangle.call(this, [x0, y0, x1, y1, x2, y2], [sx0, sy0, sx1, sy1, sx2, sy2], [m11, m12, m21, m22, dx, dy]);
		 	//uvtData.push(v0.u / rect.width, v0.v / rect.height, v1.u / rect.width, v1.v / rect.height, v2.u / rect.width, v2.v / rect.height);
			return;
		}
	...
	function rotateObject(scaled_axis, object_mat) {
		var a = Math.sqrt(jsgl.dotProduct(scaled_axis, scaled_axis));
		var angle = 0;
		for(var i = 0; i < Math.floor(Math.abs(a));++i)
		{
			var n = a > 0 ? 1 : -1;
			angle += Math.asin(n);
			a -= n;
	...
	function screenToSpherePt(x, y, camera_mat) {
